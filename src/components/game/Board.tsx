...
export default function Board({ rotateTile }: { rotateTile: (x: number, y: number) => void }) {
  const { state } = useGame();
  ...
  return (
    <div className="grid grid-cols-6 grid-rows-6 gap-1 bg-michibiki-black p-1 border-4 border-michibiki-black shadow-2xl aspect-square w-full max-w-[500px]">
      {state.board.map((row, y) =>
        row.map((cell, x) => (
          <CellComponent key={`${x}-${y}`} cell={cell} onRotate={rotateTile} />
        ))
      )}
    </div>
  );
...
function CellComponent({ cell, onRotate }: { cell: CellType; onRotate: (x: number, y: number) => void }) {
